...
bool lastCommand = "false"; // Store the last command to prevent unnecessary repetition
int stopPosition= 90;
...
void updateFirebaseStatus(bool dispenserStatus) {
...
void fetchAndUpdateFromFirebase() {
    if (httpRequestWithRetry(client, "GET", path, "", "")) {
        ...
        if (doc.containsKey("dispenserStatus")) {
            bool command = doc["dispenserStatus"].as<bool>();
