...
void loop() {
    ...
    if (currentCheck - lastCheck > 100) {
        if (servomotorOption == 1) {
          ...
          checkWeightAndControlServo(scale2, myservo2, optimalDosage, scale2Active, lastSensorWeight2, lastSensorWeight2Change);
...
void checkWeightAndControlServo(HX711& scale, Servo& servo, float dosage, bool& scaleActive, float& lastSensorWeight, unsigned long& lastSensorWeightChange) {
